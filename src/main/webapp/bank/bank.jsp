...
<jsp:useBean id="bank" class="entity.Bank" scope="application"/>
<%
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <table id="dataTable" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                    <c:forEach items="${bank.bankList}" var="bank" varStatus="status">
                                        <tr style="text-align: center">
                                            <td class="text-center">${status.index+1 }</td>
                                            <td class="text-center">${bank.BName }</td>
                                            <td class="text-center">${bank.course.CName }</td>
                                            <td class="text-center">
                                                <button type="button" class="btn bg-olive btn-xs"
                                                        onclick="location.href='${pageContext.request.contextPath}/bank/questions.jsp?bID=${bank.BID}'">详情
                                                </button>
                                                <button type="button" class="btn btn-info btn-xs" data-toggle="modal"
                                                        data-target="#myModal"
                                                        onclick="modify(${bank.BID})">修改
                                                </button>
                                                <button type="button" class="btn bg-red btn-xs "
                                                        onclick="delt(${bank.BID})">删除
                                                </button>
                                            </td>
                                        </tr>
                                    </c:forEach>
                    ...
                    <div class="modal-content" style="border-radius: 6px">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <br>
                                <br>
                                <label for="BName" class="col-sm-3 control-label">题库名称</label>
                                <div class="col-sm-9">
                                    <input id="BName" type="text" class="form-control rounded"
                                           name="BName" required="required" placeholder="题库名称"/>
                                </div>
                                <br>
                                <br>
                            </div>
...
<script>
    ...
    function modify(did) {
        ...
        $("#Submitbtn").text('修改');
        $("#title").text('修改');
        document.getElementById("BId").value = did;
    ...
    function delt(id) {
        if (confirm("您确定要删除吗？")) {
            location.href = '${pageContext.request.contextPath}/bank/deleteByPK?id=' + id;
            alert("删除成功!");
            location.reload();
        }
    }
    ...
    $("#Submitbtn").click(function () {
        ...
        if (updateorinsert == "update") {
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/bank/update", //提交的地址
                async: false,
                data: {
                    "BId": $("#BId").val(),
                    "BName": $("#BName").val(),
            ...
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/bank/add", //提交的地址
                async: false,
                data: {
                    "BName": $("#BName").val(),
                    "CourseID": $("#CourseID").val()
