...
<jsp:useBean id="point" class="entity.Point" scope="application"/>
<%
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">知识点管理</li>
                        </ol>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <table id="dataTable" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                    <c:forEach items="${point.pointList}" var="point" varStatus="status">
                                        <tr style="text-align: center">
                                            <td class="text-center">${status.index+1 }</td>
                                            <td class="text-center">${point.PName }</td>
                                            <td class="text-center">${point.course.CName }</td>
                                            <td class="text-center">
                                                <button type="button" class="btn btn-info btn-xs" data-toggle="modal"
                                                        data-target="#myModal"
                                                        onclick="modify(${point.PID})">修改
                                                </button>
                                                <button type="button" class="btn bg-red btn-xs "
                                                        onclick="delt(${point.PID})">删除
                                                </button>
                                            </td>
                                        </tr>
                                    </c:forEach>
                    ...
                    <div class="modal-content" style="border-radius: 6px">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <br>
                                <br>
                                <label for="PName" class="col-sm-3 control-label">知识点名称</label>
                                <div class="col-sm-9">
                                    <input id="PName" type="text" class="form-control rounded"
                                           name="PName" required="required" placeholder="知识点名称"/>
                                </div>
                                <br>
                                <br>
                            </div>
...
<script>
    ...
    function delt(id) {
        if (confirm("您确定要删除吗？")) {
            location.href = '${pageContext.request.contextPath}/point/deleteByPK?id=' + id;
            alert("删除成功!");
            location.reload();
        }
    }
    ...
    $("#Submitbtn").click(function () {
        ...
        if (updateorinsert == "update") {
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/point/update", //提交的地址
                async: false,
                data: {
                    "PId": $("#PId").val(),
                    "PName": $("#PName").val(),
            ...
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/point/add", //提交的地址
                async: false,
                data: {
                    "PName": $("#PName").val(),
                    "CourseID": $("#CourseID").val()
