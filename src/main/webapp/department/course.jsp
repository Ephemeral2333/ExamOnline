            ...
            <div class="card">
              <div class="card-body">
                ...
                <table id="dataTable" class="table table-bordered table-hover">
                  ...
                  <tbody>
                  <c:forEach items="${course.courseList}" var="cour" varStatus="status">
                      <tr style="text-align: center">
                        <td class="text-center">${cour.CID }</td>
                        <td class="text-center">${cour.CName }</td>
                        <td class="text-center">${cour.major.m_name }</td>
                        <td class="text-center">
                          <button type="button" class="btn bg-olive btn-xs"
                                  onclick="location.href='${pageContext.request.contextPath}/department/courseDetail.jsp?cID=${cour.CID}'">详情
                          </button>
                          <button type="button" class="btn btn-info btn-xs" data-toggle="modal"
                                  data-target="#myModal"
                                  onclick="modify('${cour.CID}')">修改
                          </button>
                          <button type="button" class="btn bg-red btn-xs "
                                  onclick="delt('${cour.CID}')">删除
                          </button>
                        </td>
                      </tr>
                  </c:forEach>
          ...
          <div class="modal-content" style="border-radius: 6px">
            ...
            <div class="modal-body">
              <div class="form-group">
                <label for="CId" class="col-sm-3 control-label">课程ID</label>
                <div class="col-sm-9">
                  <input id="CId" type="text" class="form-control rounded"
                         name="CId" required="required" placeholder="课程ID"/>
                </div>
                <br>
                <br>
                ...
                <label for="MajorID" class="col-sm-3 control-label">专业</label>
                <div class="col-sm-9">
                  <select id="MajorID" style="width: 100%;" name="MajorID" class="form-control">
                  </select>
                </div>
                <br>
                <br>
                <label for="CName" class="col-sm-3 control-label">课程名</label>
                <div class="col-sm-9">
                  <input id="CName" type="text" class="form-control rounded"
                         name="CName" required="required" placeholder="课程名"/>
                </div>
                <br>
                <br>
              </div>
...
<script>
  ...
  function modify(did) {
    ...
    var url = "${pageContext.request.contextPath}/major/findAll";
    $.get(url, function (data) {
      var html = "";
      for (var i = 0; i < data.length; i++) {
        html += "<option value=\"" + data[i].m_id + "\">" + data[i].m_name + "</option>";
      }
      $("#MajorID").html(html);
    });
  }
  ...
  function delt(id) {
    if (confirm("您确定要删除吗？")) {
      location.href = '${pageContext.request.contextPath}/course/deleteByPK?id=' + id;
      alert("删除成功!");
      location.reload();
    }
  }
  ...
  $("#Submitbtn").click(function () {
    ...
    if (updateorinsert == "update") {
      $.ajax({
        type: "POST",   //提交的方法
        url: "${pageContext.request.contextPath}/course/update", //提交的地址
        async: false,
        data: {
          "CId": $("#CId").val(),
          "CName": $("#CName").val(),
      ...
      $.ajax({
        type: "POST",   //提交的方法
        url: "${pageContext.request.contextPath}/course/add", //提交的地址
        async: false,
        data: {
          "CId": $("#CId").val(),
          "CName": $("#CName").val(),
