...
<jsp:useBean id="major" class="entity.Major" scope="application"/>
<%
  major.findAll();
%>
<html>
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">专业管理</li>
            </ol>
          </div>
        </div>
      </div>
            ...
            <div class="card">
              <div class="card-body">
                ...
                <table id="dataTable" class="table table-bordered table-hover">
                  ...
                  <tbody>
                  <c:forEach items="${major.majorList}" var="major" varStatus="status">
                    <tr style="text-align: center">
                      <td class="text-center">${status.index+1 }</td>
                      <td class="text-center">${major.department.d_name }</td>
                      <td class="text-center">${major.m_name }</td>
                      <td class="text-center">
                        <button type="button" class="btn btn-info btn-xs" data-toggle="modal"
                                data-target="#myModal"
                                onclick="modify(${major.m_id})">修改
                        </button>
                        <button type="button" class="btn bg-red btn-xs "
                                onclick="delt(${major.m_id})">删除
                        </button>
                      </td>
                    </tr>
                  </c:forEach>
          ...
          <div class="modal-content" style="border-radius: 6px">
            ...
            <div class="modal-body">
              <div class="form-group">
                ...
                <label for="Deptid" class="col-sm-3 control-label">学院</label>
                <div class="col-sm-9">
                  <select id="Deptid" style="width: 100%;" name="Deptid" class="form-control">
                  </select>
                </div>
                <br>
                <br>
                <label for="MName" class="col-sm-3 control-label">专业名称</label>
                <div class="col-sm-9">
                  <input id="MName" type="text" class="form-control rounded"
                         name="MName" required="required" placeholder="专业名称"/>
                </div>
                <br>
                <br>
              </div>
...
<script>
  ...
  function modify(did) {
    ...
    $("#Submitbtn").text('修改');
    $("#title").text('修改');
    document.getElementById("MId").value = did;

    var url = "${pageContext.request.contextPath}/dept/findAllDept";
    $.get(url, function (data) {
      var html = "";
      for (var i = 0; i < data.length; i++) {
        html += "<option value=\"" + data[i].d_id + "\">" + data[i].d_name + "</option>";
      }
      $("#Deptid").html(html);
    });
  }
  ...
  function delt(id) {
    if (confirm("您确定要删除吗？")) {
      location.href = '${pageContext.request.contextPath}/major/deleteByPK?id=' + id;
      alert("删除成功!");
      location.reload();
    }
  }
  ...
  $("#Submitbtn").click(function () {
    ...
    if (updateorinsert == "update") {
      $.ajax({
        type: "POST",   //提交的方法
        url: "${pageContext.request.contextPath}/major/update", //提交的地址
        async: false,
        data: {
          "MId": $("#MId").val(),
          "MName": $("#MName").val(),
      ...
      $.ajax({
        type: "POST",   //提交的方法
        url: "${pageContext.request.contextPath}/major/add", //提交的地址
        async: false,
        data: {
          "MName": $("#MName").val(),
          "Deptid": $("#Deptid").val()
