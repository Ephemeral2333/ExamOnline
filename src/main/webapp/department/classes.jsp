...
<jsp:useBean id="classes" class="entity.Classes" scope="application"/>
<%
    classes.findAll();
%>
<html>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <table id="dataTable" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                    <c:forEach items="${classes.classesList}" var="classes" varStatus="status">
                                        <tr style="text-align: center">
                                            <td class="text-center">${status.index+1 }</td>
                                            <td class="text-center">${classes.major.m_name }</td>
                                            <td class="text-center">${classes.CName }</td>
                                            <td class="text-center">${classes.total }</td>
                                            <td class="text-center">
                                                <button type="button" class="btn btn-info btn-xs" data-toggle="modal"
                                                        data-target="#myModal"
                                                        onclick="modify(${classes.CID})">修改
                                                </button>
                                                <button type="button" class="btn bg-red btn-xs "
                                                        onclick="delt(${classes.CID})">删除
                                                </button>
                                            </td>
                                        </tr>
                                    </c:forEach>
                    ...
                    <div class="modal-content" style="border-radius: 6px">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <br>
                                <br>
                                <label for="CName" class="col-sm-3 control-label">班级名称</label>
                                <div class="col-sm-9">
                                    <input id="CName" type="text" class="form-control rounded"
                                           name="CName" required="required" placeholder="班级名称"/>
                                </div>
                                <br>
                                <br>
                            </div>
...
<script>
    ...
    function delt(id) {
        if (confirm("您确定要删除吗？")) {
            location.href = '${pageContext.request.contextPath}/class/deleteByPK?id=' + id;
            alert("删除成功!");
            location.reload();
        }
    }
    ...
    $("#Submitbtn").click(function () {
        ...
        if (updateorinsert == "update") {
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/class/update", //提交的地址
                async: false,
                data: {
                    "CId": $("#CId").val(),
                    "CName": $("#CName").val(),
            ...
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/class/add", //提交的地址
                async: false,
                data: {
                    "CName": $("#CName").val(),
                    "MajorID": $("#MajorID").val()
