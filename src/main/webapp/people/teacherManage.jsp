...
<jsp:useBean id="teacher" class="entity.Teacher" scope="application"/>
<%
    teacher.findAll();
%>
<html>
                                ...
                                <table id="dataTable" class="table table-bordered table-hover">
                                    ...
                                    <c:forEach items="${teacher.teacherList}" var="teacher" varStatus="status">
                                        <tr style="text-align: center">
                                            ...
                                            <td class="text-center">${teacher.tea_no }</td>
                                            <td class="text-center">${teacher.tea_name }</td>
                                            <td class="text-center">${teacher.major.m_name }</td>
                                            <td class="text-center">${teacher.user.email }</td>
                                            <td class="text-center">
                                                <button type="button" class="btn btn-info btn-xs" data-toggle="modal"
                                                        data-target="#myModal"
                                                        onclick="modify('${teacher.tea_no}')">修改
                                                </button>
                                                <button type="button" class="btn bg-red btn-xs "
                                                        onclick="delt('${teacher.tea_no}')">删除
                                                </button>
                                            </td>
                                        </tr>
                                    </c:forEach>
                    ...
                    <div class="modal-content" style="border-radius: 6px">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <label for="TId" class="col-sm-3 control-label">职工号</label>
                                <div class="col-sm-9">
                                    <input id="TId" type="text" class="form-control rounded"
                                           name="TId" required="required" placeholder="职工号"/>
                                </div>
                                <br>
                                <br>
                                <label for="TName" class="col-sm-3 control-label">姓名</label>
                                <div class="col-sm-9">
                                    <input id="TName" type="text" class="form-control rounded"
...
<script>
    ...
    function modify(did) {
        ...
        $("#Submitbtn").text('修改');
        $("#title").text('修改');
        document.getElementById("TId").value = did;
    ...
    function delt(id) {
        if (confirm("您确定要删除吗？")) {
            location.href = '${pageContext.request.contextPath}/tea/deleteByPK?id=' + id;
            alert("删除成功!");
            location.reload();
        }
    }
    ...
    $("#Submitbtn").click(function () {
        ...
        if (updateorinsert == "update") {
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/tea/update", //提交的地址
                async: false,
                data: {
                    "TId": $("#TId").val(),
                    "TName": $("#TName").val(),
            ...
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/tea/add", //提交的地址
                async: false,
                data: {
                    "TId": $("#TId").val(),
                    "TName": $("#TName").val(),
