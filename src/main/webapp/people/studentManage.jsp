...
<jsp:useBean id="student" class="entity.Student" scope="application"/>
<%
    student.findAll();
%>
<html>
...
<%--                                <input type="hidden" name="SId" id="SId">--%>
                                ...
                                <label for="SId" class="col-sm-3 control-label">学号</label>
                                <div class="col-sm-9">
                                    <input id="SId" type="text" class="form-control rounded"
                                           name="SId" required="required" placeholder="学号"/>
                                </div>
                                <br>
                                <br>
                                <label for="SName" class="col-sm-3 control-label">姓名</label>
                                <div class="col-sm-9">
                                    <input id="SName" type="text" class="form-control rounded"
                                           name="SName" required="required" placeholder="学生姓名"/>
                                </div>
                                <br>
                                <br>
                                <label for="ClassID" class="col-sm-3 control-label">班级</label>
                                <div class="col-sm-9">
                                    <select id="ClassID" style="width: 100%;" name="ClassID" class="form-control">
                                    </select>
                                </div>
                                <br>
                                <br>
...
<script>
    ...
    function modify(did) {
        ...
        $("#Submitbtn").text('修改');
        $("#title").text('修改');
        document.getElementById("SId").value = did;

        var url = "${pageContext.request.contextPath}/class/findAll";
    ...
    function delt(id) {
        if (confirm("您确定要删除吗？")) {
            location.href = '${pageContext.request.contextPath}/stu/deleteByPK?id=' + id;
            alert("删除成功!");
            location.reload();
        }
    }
    ...
    $("#Submitbtn").click(function () {
        ...
        if (updateorinsert == "update") {
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/stu/update", //提交的地址
                async: false,
                data: {
                    "SId": $("#SId").val(),
                    "SName": $("#SName").val(),
            ...
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/stu/add", //提交的地址
                async: false,
                data: {
                    "SId": $("#SId").val(),
                    "SName": $("#SName").val(),
